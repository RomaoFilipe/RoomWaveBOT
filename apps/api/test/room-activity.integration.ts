...
try{
 ...
 globalThis.fetch=async input=>Response.json({status:"success",denormalized:{[String(input)]:{data:{name:"Owned test",customers_id:seed,customers_room_id:1,owner_avatarname:"Owner"}}}});const owner=await prisma.user.create({data:{username:'Activity test owner',imvuUserId:seed}});users.push(owner.id);
 const admin=await prisma.user.create({data:{username:'Activity test admin',imvuUserId:seed+'a'}});users.push(admin.id);
 const room=await prisma.room.create({data:{name:'Activity test',imvuRoomId:seed+'-1',members:{create:[{userId:owner.id,role:'OWNER'},{userId:admin.id,role:'ADMIN'}]}}});roomId=room.id;
