...
import {lookupUser,lookupRoom,lookupProfile,lookupCatalog,lookupOutfits,lookupRooms} from '../../../../tools/imvu-directory.mjs';
const schema=z.object({roomId:z.string().uuid(),imvuUserId:z.string().min(1).max(64),kind:z.enum(['user','room','profile','catalog','outfits','rooms']),provider:z.enum(['imvu','ankh']).default('imvu'),query:z.string().trim().min(1).max(300)});
export async function imvuDirectoryRoutes(app:FastifyInstance){
 ...
 app.post('/imvu/lookup',async(req,reply)=>{
  ...
  busy++;
  try{
   const lookups={user:lookupUser,room:lookupRoom,profile:lookupProfile,catalog:lookupCatalog,outfits:lookupOutfits,rooms:lookupRooms};
   return {result:await (provider==='ankh'&&(kind==='user'||kind==='room')?lookupAnkh(kind,query):lookups[kind](query))};
  }
