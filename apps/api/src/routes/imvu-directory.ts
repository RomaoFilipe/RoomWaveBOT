...
const schema=z.object({roomId:z.string().uuid(),imvuUserId:z.string().min(1).max(64),kind:z.enum(['user','room']),query:z.string().trim().min(1).max(300)});
export async function imvuDirectoryRoutes(app:FastifyInstance){
 ...
 app.post('/imvu/lookup',async(req,reply)=>{
  ...
  const {roomId,imvuUserId,kind,query}=parsed.data;
  ...
  busy++;
  try{return {result:await (kind==='user'?lookupUser(query):lookupRoom(query))};}
  catch(e){const error=e instanceof Error?e.message:'';return reply.code(error==='IMVU_NOT_FOUND'?404:400).send({error:/^(IMVU_|INVALID_IMVU_)/.test(error)?error:'IMVU_UNAVAILABLE'});}
  finally{busy--;}
 });
}
