...
$('lookup-form').onsubmit=async event=>{
 ...
 try{
  const {result:r}=await api('imvu/lookup',{kind:form.elements.kind.value,query:form.elements.query.value.trim()});
  box.replaceChildren(element('h2',r.kind==='user'?r.displayName:r.name));
  const rows=r.kind==='user'?[['Username',r.username],['CID',r.id],['Conta criada',r.created?new Date(r.created).toLocaleDateString('pt-PT'):null],['Descrição',r.tagline],['Creator',r.isCreator?'Sim':'Não'],['VIP',r.isVip?'Sim':'Não']]:[['ID',r.id],['Dono',r.ownerName],['CID do dono',r.ownerId],['Descrição',r.description],['Ocupação',r.occupancy===null?null:r.occupancy+' / '+(r.capacity??'—')],['Acesso',r.privacy],['Idioma',r.language],['AP',r.isAp?'Sim':'Não']];
  ...
  box.append(element('p','Fonte: IMVU · '+new Date(r.fetchedAt).toLocaleTimeString('pt-PT')+' · Cache até 1 minuto','muted small'));
