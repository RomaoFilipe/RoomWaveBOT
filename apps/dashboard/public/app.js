...
for(const nav of document.querySelectorAll('[data-tab]'))nav.onclick=()=>{tab=nav.dataset.tab;for(const node of document.querySelectorAll('.page'))node.hidden=node.id!==tab;for(const node of document.querySelectorAll('[data-tab]'))node.classList.toggle('active',node===nav);$('page-name').textContent={overview:'Visão geral',rooms:'Salas',commands:'Comandos',logs:'Registo de atividade'}[tab];if(tab==='rooms')loadRooms();if(tab==='commands')loadCommands();if(tab==='logs')loadLogs();};
