...
const errors={IMVU_OWNER_MISMATCH:"Esta sala pertence a outra conta IMVU.",IMVU_OWNERSHIP_REQUIRED:"A recolha exige confirmar que és o proprietário da sala no IMVU. Consulta a aba Segurança.",WATCH_LIMIT:"Limite de 50 pessoas destacadas por sala.",ANKH_UNAVAILABLE:"O serviço Ankh não está disponível.",ANKH_UPSTREAM_ERROR:"O Ankh não conseguiu consultar estes dados no IMVU. Podes tentar a fonte IMVU direta.",ANKH_BUSY:"O Ankh está ocupado. Tenta novamente daqui a pouco.",IMVU_NOT_FOUND:"Não foi encontrado um resultado exato.",IMVU_RESTRICTED:"O IMVU não disponibiliza estes dados nesta consulta.",IMVU_RATE_LIMIT:"Demasiadas consultas. Aguarda um pouco.",IMVU_UNAVAILABLE:"O IMVU está temporariamente indisponível.",INVALID_IMVU_USER:"Usa o nome de utilizador, CID ou link de perfil IMVU.",INVALID_WELCOME:"Escreve uma mensagem entre 1 e 350 caracteres.",OWNER_ONLY:"Só o dono pode alterar esta configuração.",INVALID_ROOM:"Preenche o nome e o link/ID da sala.",INVALID_IMVU_ROOM:"Usa um link IMVU de sala ou o ID no formato 12345-678.",ROOM_NOT_OWNED:"Não tens controlo desta sala no RoomWave.",ROOM_ALREADY_EXISTS:"Essa sala já está registada.",ROOM_SWITCH_IN_PROGRESS:"A mudança de sala está em curso. Aguarda a confirmação.",INVALID_LOGIN:'Utilizador ou senha incorretos.',TOO_MANY_ATTEMPTS:'Demasiadas tentativas. Volta a tentar dentro de 15 minutos.',OWNER_NOT_CONFIGURED:'O dono da sala não está configurado.',SERVICE_UNAVAILABLE:'Não foi possível contactar o serviço. Tenta novamente.',ALREADY_EXISTS:'Esse comando já existe.',NOT_FOUND:'Esse comando já não existe.',RESERVED_OR_INVALID_NAME:'Nome reservado ou inválido. Os comandos existentes estão protegidos.',INVALID_COMMAND:'Confirma o nome e o texto do comando.',ACTION_IN_PROGRESS:'Há uma ação em curso. Aguarda um momento.',PLAYBACK_NOT_READY:'A reprodução não está pronta para essa ação.',INVALID_QUERY:'Escreve um nome de música ou link válido.'};
...
for(const nav of document.querySelectorAll('[data-tab]'))nav.onclick=()=>{tab=nav.dataset.tab;for(const node of document.querySelectorAll('.page'))node.hidden=node.id!==tab;for(const node of document.querySelectorAll('[data-tab]'))node.classList.toggle('active',node===nav);$('page-name').textContent={overview:'Visão geral',rooms:'Salas',security:'Segurança',lookup:'Ferramentas IMVU',commands:'Comandos',logs:'Registo de atividade'}[tab];if(tab==='rooms')loadRooms();if(tab==='security')loadSecurityRooms();if(tab==='lookup')loadActivity();if(tab==='commands'){loadCommands();loadWelcome();}if(tab==='logs')loadLogs();};
...
setInterval(()=>{if(signedIn&&!document.hidden){refresh();if(tab==='rooms')loadRooms();if(tab==='security')loadSecurityView();if(tab==='logs'&&$('auto-logs').checked)loadLogs();}$('clock').textContent=new Date().toLocaleTimeString('pt-PT',{hour:'2-digit',minute:'2-digit'});},8000);
...
$('lookup-form').onsubmit=async event=>{
 ...
 try{
  ...
  if(r.kind==='room'){
   const save=element('button','Usar em Salas','secondary');save.onclick=()=>{$('room-form').elements.name.value=r.name;$('room-form').elements.location.value=r.id;document.querySelector('[data-tab=rooms]').click();$('room-form').scrollIntoView({behavior:'smooth'});};box.append(save);
  }
...
let securityLoading=false,securityViewing=false,securityData=null,securitySelected=null;
async function securityApi(payload){return api('security',payload);}
async function loadSecurityRooms(){
 if(securityLoading)return;securityLoading=true;
 try{
  const data=await securityApi({action:'rooms'}),select=$('security-room');const previous=select.value;select.replaceChildren();$('security-rooms-status').replaceChildren();
  for(const room of data.rooms){
   if(room.verified){const option=element('option',room.name+' · '+room.imvuRoomId);option.value=room.id;select.append(option);}
   else $('security-rooms-status').append(element('p',room.name+' — '+(room.reason==='IMVU_OWNER_MISMATCH'?'proprietário IMVU diferente':'não foi possível confirmar a propriedade')));
  }
  if([...select.options].some(o=>o.value===previous))select.value=previous;
  if(!select.options.length){$('security-rooms-status').append(element('p','Adiciona uma sala tua para começar.'));$('security-detail').hidden=true;securitySelected=null;}
  else await loadSecurityView(true);
 }catch(e){notice(e.message,true);}finally{securityLoading=false;}
}
async function loadSecurityView(reset=false){
 const roomId=$('security-room').value;if(!roomId||securityViewing)return;securityViewing=true;
 try{
  const data=await securityApi({action:'view',roomId});if(roomId!==$('security-room').value)return;securityData=data;
  $('security-detail').hidden=false;$('security-title').textContent=data.room.name;$('security-owner').textContent='Proprietário confirmado: '+data.room.ownerName+' · CID '+data.room.ownerId;
  if(reset||securitySelected!==roomId)$('security-monitor').elements.enabled.checked=data.settings.enabled;securitySelected=roomId;
  $('security-live-state').textContent=!data.settings.enabled?'Recolha desligada.':!data.active?'Sala configurada; o bot está noutra sala.':data.snapshot?'Participantes observados às '+new Date(data.snapshot.at).toLocaleTimeString('pt-PT'):'Sem observação recente. Aguarda o bot ou verifica a ligação.';
...
function renderSecurityEvents(){
 const box=$('security-events');box.replaceChildren();const events=(securityData?.events||[]).filter(e=>!$('security-highlight-only').checked||e.highlighted);
 for(const e of events){const row=element('div',undefined,'activity-row'+(e.highlighted?' security-highlight':''));row.append(element('small',new Date(e.time).toLocaleString('pt-PT')+' · '+({message:'Mensagem',join:'Entrou',leave:'Saiu',baseline:'Observação iniciada'}[e.type]||e.type)),element('strong',e.name||e.userId||''),element('p',e.text||''));box.append(row);}if(!events.length)box.append(element('p','Sem registos para este filtro.','muted'));
}
$('security-highlight-only').onchange=renderSecurityEvents;
$('security-room').onchange=()=>{securityData=null;$('security-detail').hidden=true;loadSecurityView(true);};
$('security-refresh').onclick=loadSecurityRooms;
...
async function loadModeration(more=false){
 ...
 try{
  ...
  for(const e of data.events){const row=element('div',undefined,'activity-row');row.append(element('small',new Date(e.createdAt).toLocaleString('pt-PT')+' · '+(e.action==='WARN'?'Aviso':'Expulsão')+' · '+({REQUESTED:'Pedido',CONFIRMED:'Confirmado',FAILED:'Falhou',UNCONFIRMED:'Não confirmado'}[e.status]||e.status)),element('strong',e.actorName+' ('+e.actorCid+') → '+e.targetName+' ('+e.targetCid+')'),element('p',e.reason));if(e.resultCode)row.append(element('small',({CHAT_ECHO:'Publicação confirmada pelo eco do chat',SEND_FAILED:'Falha de envio',ECHO_TIMEOUT:'Sem confirmação do chat',ROOM_CHANGED:'Sala alterada',KICK_CONFIRMED:'Expulsão aceite pelo IMVU e ausência confirmada',KICK_REJECTED:'IMVU recusou a expulsão',KICK_UNCONFIRMED:'Resultado da expulsão não confirmado',KICK_PREFLIGHT_FAILED:'Verificação da expulsão falhou',TARGET_ABSENT:'Destinatário já ausente',KICK_UNAVAILABLE:'Expulsão nativa indisponível',INTERRUPTED:'Operação interrompida ou sem resultado'}[e.resultCode]||'Resultado indisponível')));box.append(row);}
